...
export async function updateLenderDetail(
  id: number,
  data: {
    formData: {
      title1: string;
  ...
  try {
    await prisma.lender.update({
      ...
      data: {
        ...
        isVisibleMain: data.formData.isVisibleMain,
        isVisibleArea: data.formData.isVisibleArea,
        isVisibleLoan: data.formData.isVisibleLoan,
        ...
        regionKinds: {
          set: data.regionKindIds.map((id) => ({
            id,
          })),
        },

        loanKinds: {
          set: data.loanKindIds.map((id) => ({
            id,
          })),
        },
      },
