...
import { updateLenderDetail } from "@/actions/admin/update-lender-detail";
...
type Props = {
  data: GetLenderDetail;
  regionKinds: GetRegionKinds["regionKinds"];
  loanKinds: GetLoanKinds["loanKinds"];
};

export default function Detail({ data, regionKinds, loanKinds }: Props) {
  ...
  const [formData, setFormData] = useState({
    ...
    isVisibleMain: data?.isVisibleMain || false,
    isVisibleArea: data?.isVisibleArea || false,
    isVisibleLoan: data?.isVisibleLoan || false,
  ...
  const [selectedRegions, setSelectedRegions] = useState<number[]>(
    data?.regionKinds.map((item) => item.id) || []
  );
  const [selectedLoans, setSelectedLoans] = useState<number[]>(
    data?.loanKinds.map((item) => item.id) || []
  );
  ...
  const handleSave = async () => {
    ...
    startTransition(async () => {
      const result = await updateLenderDetail(Number(params.id), {
        formData,
        lenderInfo,
